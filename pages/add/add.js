...
const LEGACY_DRAFT_KEY = "addInspirationDraft";

function clearLegacyDraft() {
  try {
    const draft = wx.getStorageSync(LEGACY_DRAFT_KEY);
    const images = draft && Array.isArray(draft.images) ? draft.images : [];
    images.filter(Boolean).forEach(imageUtils.removeSavedImage);
    wx.removeStorageSync(LEGACY_DRAFT_KEY);
  } catch (error) {
    console.warn("clear legacy add inspiration draft failed", error);
  }
}
